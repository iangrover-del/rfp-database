...
# RFP Database Deployment Script
set -e

echo "🚀 Starting RFP Database deployment..."
...
aws ecr create-repository --repository-name $STACK_NAME-repo --region $AWS_REGION || echo "Repository may already exist"

# Get ECR login token
aws ecr get-login-password --region $AWS_REGION | docker login --username AWS --password-stdin $(aws sts get-caller-identity --query Account --output text).dkr.ecr.$AWS_REGION.amazonaws.com

# Build Docker image
docker build -t $STACK_NAME-repo .

# Tag and push image
ECR_URI=$(aws sts get-caller-identity --query Account --output text).dkr.ecr.$AWS_REGION.amazonaws.com/$STACK_NAME-repo:latest
docker tag $STACK_NAME-repo:latest $ECR_URI
docker push $ECR_URI
...
echo "  4. Configure Slack integration (optional)"
echo ""
echo "📚 Documentation:"
echo "  - API Documentation: $APPLICATION_URL/docs"
echo "  - Streamlit Interface: $APPLICATION_URL:8501"
echo ""
